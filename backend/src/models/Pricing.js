...
const pricingSchema = new mongoose.Schema({
  ...
  status: { 
    type: String, 
    enum: ['stable', 'optimal', 'expiring_soon', 'critical', 'low_stock', 'out_of_stock'],
    default: 'stable'
  },
...
pricingSchema.pre('save', function(next) {
  ...
  if (this.currentPrice && this.cost) {
    this.margin = Math.round(((this.currentPrice - this.cost) / this.currentPrice) * 100 * 10) / 10;
  }
  ...
  if (this.originalPrice && this.currentPrice) {
    const change = ((this.currentPrice - this.originalPrice) / this.originalPrice) * 100;
    this.priceChange = `${change >= 0 ? '+' : ''}${Math.round(change * 10) / 10}%`;
  }
